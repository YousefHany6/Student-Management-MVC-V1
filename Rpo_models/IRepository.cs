...
namespace FStudentManagement.Rpo_models
{
 ...
 void Detach(T entity);
  ...
  Task<bool> GetCourseForTeacherExists(int teacherId, int courseId);
  Task<List<T>> GetFilteredAsync(Expression<Func<T, bool>> filter,
                                                   params Expression<Func<T, object>>[] includes);
  Task<List<TeacherCourse>> teacherforcourse(int id);
  Task<TeacherCourse> GetByTeacherAndCourseId(int teacherId, int courseId);
