...
namespace FStudentManagement.Rpo_models
{
 public class Repository<T> : IRepository<T> where T : class
 {
  ...
  public async Task<bool> GetCourseForTeacherExists(int teacherId, int courseId)
  {
   return await _context.Set<TeacherCourse>()
       .AnyAsync(tc => tc.Teacher_ID == teacherId && tc.Course_ID == courseId);
  }
  ...
  public async Task<List<TeacherCourse>> teacherforcourse(int id)
  {


   return await _context.TeacherCourses
   .Where(tc => tc.Course_ID == id)
   .ToListAsync();
  }
		...
		public async Task<TeacherCourse> GetByTeacherAndCourseId(int teacherId, int courseId)
		{
			return await _context.TeacherCourses
							.FirstOrDefaultAsync(tc => tc.Teacher_ID == teacherId && tc.Course_ID == courseId);
		}
